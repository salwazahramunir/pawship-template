...
import { ChevronLeft, ChevronRight, Play, Expand, Download } from "lucide-react"
...
export function ProductImageGallery({ images, videos = [], productName }: ProductImageGalleryProps) {
  ...
  const currentMedia = isVideoMode ? videos[currentVideoIndex] : images[currentIndex]
        ...
        {allMedia.map((media, index) => (
          <button
            ...
            className={`relative aspect-square rounded-lg overflow-hidden border-2 transition-all ${
              (media.type === "image" && !isVideoMode && media.index === currentIndex) ||
              (media.type === "video" && isVideoMode && media.index === currentVideoIndex)
                ? "border-primary"
